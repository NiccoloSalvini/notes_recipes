...
conference where the author of the package [MaxKuhn](https://resources.rstudio.com/authors/max-kuhn) explains the
...
the _recipe_. Once you know the _recipe_ which is basically a sequence of
...
```{r remedy01, warning=FALSE, include=FALSE}
...
cook through the steps with any ingredients. Say you are cooking chicken masala, you know the recipe beacuse you love indian food. Once you have to make lamb masala, assuming that there is no indian chef hearing, it is just a matter of coocking meat time. 
...
here you define the design matrix, so the set of all the predictors, but we havent finished yet.
...
```{r remedy05, echo=FALSE, error=FALSE, eval=FALSE}
...
```{r image_pca_plot, fig.keep="none", imgcenter = TRUE, eval=FALSE}
pca[1:4, 1:8]
...
```{r image_pca_fig, echo = FALSE, fig.width = 5.5, fig.height = 5.6, eval=FALSE} 
...
knitr::include_graphics("img/image_pca_fig-1.png")
```
...
```{r image_kpca_fig, echo = FALSE, fig.width = 5.5, fig.height = 5.6, eval=FALSE}
...
```{r, echo=FALSE, imgcenter  = TRUE}
knitr::include_graphics("img/image_kpca_fig-1.png")
```
...
```{r image_dists_fig, echo = FALSE, fig.width = 5.5, fig.height = 5.6, eval=FALSE}
...
```{r, echo=FALSE, imgcenter  = TRUE}
knitr::include_graphics("img/image_dists_fig-1.png")
```
